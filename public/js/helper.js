...
//可拖拉，变尺寸弹出框
function popup_window(load_url, title_txt, popup_width, popup_height, fresh_window, relative_position_x, relative_position_y,no_sizeable,no_showMax)
{
	if (!title_txt) title_txt = "";
	...
	$("#"+domId).dialog({
		...
		open: function(event, ui) {
			...
			if (!popup_height) {
				...
				$("#"+domId).append("<iframe onload='iFrameWidthHeight(\""+domId+"\",false,1);' name='showContents' allowtransparency='yes' frameborder='no' style='border:1px solid #99bbe8' scrolling='auto' id='showContents2' height="+(popup_height-68)+" width="+(popup_width-40)+"  src='./admin/template/view/frontpage/loading.php'></iframe>");
